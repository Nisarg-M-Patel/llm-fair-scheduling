...
class DataSeries:
    ...
    def plot_differential(self, path: str, plot_name: str, x_window: float = 30.0) -> None:
        """
        Plot the differential (rate of change) using a sliding window approach.
        ...
        plot_name : str
            Name of the plot
        x_window : float
            Size of the sliding window in x-axis units (default: 30.0)
        """
        if len(self._data_series) == 0:
            return
        ...
        # Sort by x values to ensure correct differential calculation
        df = df.sort_values(self._x_name)
        
        # take cumsum of y values
        df[self._y_name] = df[self._y_name].cumsum()
        ...
        df[differential_col_name] = dy / dx
        
        # Apply rolling mean to smooth the rates
        window_size = max(2, int(len(df) * (x_window / (df[self._x_name].max() - df[self._x_name].min()))))
        df[differential_col_name] = df[differential_col_name].rolling(
            window=window_size, 
            center=True,
            min_periods=2
        ).mean()
            ...
            wandb.log(
                {
                    f"{plot_name}_differential": wandb.plot.line(
                        wandb.Table(dataframe=df_clean),
                        self._x_name,
        ...
        # Save dataframe
        self._save_df(df_clean, path, plot_name)