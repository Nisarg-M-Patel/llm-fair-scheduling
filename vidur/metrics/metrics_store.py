...
from vidur.metrics.constants import (
    ...
    ClientCompletionMetricsTimeSeries,
...
class MetricsStore:
    ...
    def __init__(self, simulation_config: SimulationConfig) -> None:
        ...
        self._client_completion_metrics_time_series: Dict[
            ClientCompletionMetricsTimeSeries, DataSeries
        ] = {}
        for metric_name in ClientCompletionMetricsTimeSeries:
            self._client_completion_metrics_time_series[metric_name] = DataSeries(
                TIME_STR,
                metric_name.value,
                self._config.subsamples,
                self._config.save_table_to_wandb,
    ...
    def _store_completion_metrics(self, base_plot_path: str):
        ...
        for dataseries in self._client_completion_metrics_time_series.values():
            dataseries.plot_step(
                base_plot_path, f"{dataseries._y_name}_time_series", SERVICE_STR
            )
            dataseries.plot_differential(
                base_plot_path, f"{dataseries._y_name}_rate"
            )
    ...
    def _on_request_end(self, time: float, request: Request) -> None:
        ...
        self._request_metrics_histogram[
            RequestMetricsHistogram.CLIENT_ID
        ].put(request.id, request.client_id)
        ...
        if request.client_id == 0:
            self._client_completion_metrics_time_series[
                ClientCompletionMetricsTimeSeries.CLIENT_ZERO_SERVICE_RECEIVED
            ].put(request.completed_at, request.service_cost)
        else:
            self._client_completion_metrics_time_series[
                ClientCompletionMetricsTimeSeries.CLIENT_ONE_SERVICE_RECEIVED
            ].put(request.completed_at, request.service_cost)
