...
@dataclass
class BaseClientAssignmentGeneratorConfig(BasePolyConfig):
    seed: int = field(
        default=42,
        metadata={"help": "Seed for the random number generator."},
    )
...
@dataclass
class FixedClientAssignmentGeneratorConfig(BaseClientAssignmentGeneratorConfig):
    @staticmethod
    def get_type():
        return ClientAssignmentGeneratorType.FIXED


@dataclass
class BinomialClientAssignmentGeneratorConfig(BaseClientAssignmentGeneratorConfig):
    activation_rate: float = field(
        default=0.5,
        metadata={"help": "Activation rate for Binomial Client Assignment Generator."},
    )

    @staticmethod
    def get_type():
        return ClientAssignmentGeneratorType.BINOMIAL
...
class SyntheticRequestGeneratorConfig(BaseRequestGeneratorConfig):
    ...
    client_assignment_generator_config: BaseClientAssignmentGeneratorConfig = field(
        default_factory=BinomialClientAssignmentGeneratorConfig,
        metadata={
            "help": "Client assignment generator config for Synthetic Request Generator."
        },
    )
...
@dataclass
class VTCSchedulerConfig(BaseReplicaSchedulerConfig):
    max_tokens_in_batch: int = field(
        default=4096,
        metadata={"help": "Maximum tokens in batch for VTC."},
    )
    client_zero_damping: float = field(
        default=1,
        metadata={"help": "Client zero weight for VTC."},
    )
    client_one_damping: float = field(
        default=1,
        metadata={"help": "Client one weight for VTC."},
    )

    @staticmethod
    def get_type():
        return ReplicaSchedulerType.VTC

@dataclass
class VTCSarathiSchedulerConfig(BaseReplicaSchedulerConfig):
    ...
    @staticmethod
    def get_type():
        return ReplicaSchedulerType.VTC_SARATHI
...
class ClusterConfig:
    ...
    replica_scheduler_config: BaseReplicaSchedulerConfig = field(
        default_factory=VTCSchedulerConfig,
